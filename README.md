...
Xcode 工程、运行说明、DMG 与 Sparkle appcast 打包脚本位于 [`SimuBoardMac`](SimuBoardMac/README.md)。1.1.2 为所有统计热力图加入随当前数据自动变化的连续色阶，并让历史格子的日期、时段和字符数在悬停时立即显示、点击后保持显示；构建产物位于 `SimuBoardMac/build/Battuta-1.1.2-unnotarized.dmg`。
