...
On the keycap, use a small rounded mechanical-switch cross stem in the center, with one thick rounded acoustic arc on its left and one matching arc on its right. The arcs face outward as sound radiating horizontally from the switch. Avoid literal letters, music notes, chain-link forms, Wi-Fi/RSS shapes, text, and trademarks.

## Transparency refinement

Remove only the outer canvas beyond the charcoal rounded-square icon body and replace it with genuine alpha transparency. Preserve every dark region inside the icon, all materials, lighting, shadows, proportions, and smooth antialiased outer edges; leave no black halo.
