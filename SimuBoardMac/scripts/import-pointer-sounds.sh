...
render_profile() {
  local profile=$1
  local style_filter=$2

  render_sample "$PRESS_SOURCE" "$TARGET_ROOT/$profile/press/PRIMARY.wav" "$style_filter"
  render_sample "$RELEASE_SOURCE" "$TARGET_ROOT/$profile/release/PRIMARY.wav" "$style_filter"
}

# The names describe simulated tonal treatments, not recordings of particular
# mouse brands or switches. Output levels retain ample headroom after filtering.
render_profile classic "volume=-3dB"
render_profile silent "lowpass=f=3200:p=2,volume=8dB"
render_profile crisp "asetrate=50274,aresample=48000,highpass=f=700,equalizer=f=4800:t=q:w=1.1:g=3,volume=-7dB"
render_profile heavy "asetrate=38808,aresample=48000,lowpass=f=5200:p=2,bass=g=3:f=180:w=0.7,volume=3dB"
render_profile glass "asetrate=54000,aresample=48000,highpass=f=1200,equalizer=f=6500:t=q:w=1:g=4,volume=-9dB"
