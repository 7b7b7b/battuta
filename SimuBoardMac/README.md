...
键盘绝对音量会在系统未静音且有效输出音量不为 0 时尽量保持应用内设定的键盘响度；如果 macOS 已静音或当前有效输出音量为 0，则不会播放键盘声音。为了避免在极低系统音量下把短瞬态键音前级放大到失真，补偿会按当前音色包的最高样本峰值与应用内音量自动收敛，共享键盘总线也会限制重叠按键的峰值。点击音量仍按应用内滑杆正常工作，不会反向补偿系统主音量。
